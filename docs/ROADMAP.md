...
*Active backlog + recent shipped history. Older shipped/superseded items are in [ROADMAP_archive.md](ROADMAP_archive.md).*
...
#### BL-20 — Auto-resolve ambiguous dispatch slots from the API's billed cost (fewer review prompts)  ·  *follow-up to #322*  ·  **SHIPPED (4.5.5) — as BL-53**
*Shipped as the measured-cost reconciliation: a settled dispatched slot the heuristic can't decide is priced from Octopus's billed cost (band from the bill; `imp_rate` snapped to the clean tariff band; `imp_cost` = the bill), and a material disagreement is applied AND review-flagged as a possible billing discrepancy rather than silently accepted. See the 4.5.5 design doc.*
...
#### BL-50 — Unify user-job mutual exclusion (deny + idempotent-retry)  ·  *architecture · correctness · target v5.0.0*
...
#### BL-61 — 4.5.7 settlement/chart cleanup follow-ups  ·  *tech-debt · target v5.0.0*
...
#### BL-47 — Auto-run recorder device attribution across freshly gap-filled windows  ·  *attribution · medium*
...
#### BL-51 — Reprice banner should distinguish an upgrade from an import-triggered reprice  ·  *UI · low · not 4.5.4*
