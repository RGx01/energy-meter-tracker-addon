...
#### Deprecation-CI coverage for the Measurements API + drift guard  ·  *maintainer tooling*
*The weekly GraphQL-deprecation check silently missed the historical-import query.* The check (`.build/check_graphql_deprecations.py` + `graphql-deprecations.yml`) matches the **live schema** against **hand-maintained** field sets in `kraken_api_client.py` (`_EMT_GRAPHQL_FIELDS` / `_EMT_GRAPHQL_TYPED_FIELDS` / `_EMT_GRAPHQL_ENUMS`). When the 3.5.0 **`get_measurements`** query was added (the whole backfill — kWh + billed cost + off/peak label), its fields were **not** added to those sets, so an upstream deprecation of e.g. `measurements` / `statistics` / `costInclTax` would never have opened an issue. **Done (4.0.0):** the billing-critical Measurements output fields + filter enums (`THIRTY_MIN_INTERVAL`, `CONSUMPTION`, `GENERATION`) registered; **and a completeness test** (`TestGraphqlDeprecationSetCompleteness` in `test_kraken_client.py`) that AST-parses every GraphQL query string in `kraken_api_client.py` and asserts each selected output field is registered, ignored, or in a documented generic allowlist — the drift guard. *(It immediately surfaced two more previously-uncovered queries: the **rate-limit pacing** query — `pointsAllowanceRateLimit`/`usedPoints`/`remainingPoints`/`isBlocked` — and IOG `currentState`; all now registered.)* **Still to do:** (1) **type-scoped matching** (`_EMT_GRAPHQL_TYPED_FIELDS`) for the Relay-connection generics still in the test's allowlist (`edges`/`node`/`value`/`unit`/`pageInfo`/`hasNextPage`/`endCursor`/`limit`/`ttl`) so they're covered without bare-name false positives; (2) note in the doc that the check covers **GraphQL only** — the historical import's **REST** dependencies (`get_account` property `postcode`/`moved_in_at`/`moved_out_at`/address, `get_consumption`) and the external **Carbon Intensity API** are out of scope and would need their own light shape/contract guard.

#### Old-agreement rate/standing-charge fetch sends `period_from > period_to` (HTTP 400)  ·  *fix*
*Import pricing fails for pre-import-floor agreements.* On the 4.0.0 re-import (back to 2024-07), building rate + standing-charge schedules for **old** products emitted `HTTP 400 … {"period_from":["Must not be greater than \`period_to\`."]}` for e.g. `VAR-22-11-01` and `OUTGOING-FIX-12M-19-05-13` — the tariff `period_from`/`period_to` window handed to the Octopus REST tariff endpoints is **inverted** for agreements whose validity sits (partly) outside the imported span. Those schedules come back empty, so blocks in those windows fall back to last-known/CSV-derived rates rather than the correct historical tariff. (Surfaced only because the 4.0.0 log now prints the HTTP status + body instead of a blank `fetch failed:`.) Fix: clamp/skip the schedule fetch to the agreement's own `[valid_from, valid_to)` ∩ the requested window, and never issue a request with `from >= to`.

#### #307 — Universal block-plausibility guard + sensor-type-aware reset handling  ·  *fix*
*A physically-impossible block survived every guard.* The 2026-07 power-cut reboot storm booked **6137 kWh** on the `house_battery` sub-meter in a single half-hour. Root cause: the block was written as an **unsettled gap block** via a recovery/reconstruction path that bypasses `compute_channel` (where the `_ROGUE_SUB_BLOCK_KWH_CEILING` = 60 kWh clamp lives), and the parent-import invariant (PASS-2 / **BL-19**) *did* see it (log: `house_battery … 6137.5920 kWh EXCEEDS parent grid import 0.0000 … recording as-is (settlement will re-clamp)`) but **deferred** because the block was unsettled. Two guards, neither clamped. The stored history already proves the seed was wrong — the previous block's `read_end` was 6309, so a 6137 delta is impossible. Plan:
1. **Move the physical-plausibility ceiling to the single block-*write* point** — finalise, gap-fill, reconstruction, restart-recovery, *all* paths — not just live capture. Derive the ceiling from block length (`block_minutes × max-plausible-kW`) rather than the flat 60/500.
2. **Cumulative-register continuity check** using the previously-stored `read_end`: a backward jump or implausible delta ⇒ reset/glitch ⇒ book 0 and re-baseline from `read_end`. **Sensor-type-aware — critical:** apply the monotonic / backwards-is-bad rule ONLY to cumulative lifetime registers (e.g. `solax…in_aggregated_energy`), NEVER to session-energy sensors (e.g. `zappi_charge_added_session`), which legitimately reset to 0 on each EV unplug/replug. Today EMT only "gets away with it" via time-separation between charges — make the sensor semantic **explicit** (per-channel cumulative vs session) instead of implicit.
3. **Never defer a physical impossibility:** PASS-2 may keep deferring the *fine* device-vs-parent clamp to settlement, but a physically-impossible value must be coarse-clamped **immediately**, with settlement only refining the sane figure.

**Built (4.0.0):** §1 + §3 shipped — a physical-plausibility guard (`_max_plausible_block_kwh` = `block_minutes/60 × 120 kW`) now lives at the shared write point `_apply_pass2` (finalise **and** gap-fill), clamping an impossible sub-meter delta to 0, baselining `read_start` onto `read_end`, and flagging `needs_review` **immediately — never deferred** (fixes the exact `clamp_overflow=False` deferral that let 6137 through). Sensor-agnostic, so it can't mis-fire on a session sensor. The existing-data cleanup is a **run-once self-heal** in `engine_startup` (`block_store.sweep_implausible_sub_blocks`, marker-gated by `_SPIKE_SWEEP_DONE_KEY`) — no UI: it detects the impossibility from the register itself and repairs automatically, leaving `imp_kwh_grid`/`imp_cost` (billing) untouched and recomputing device carbon (validated on the prod DB: the one 6137 kWh block → 0, DB carbon 1.87M → 1.01M g). A read-only `/api/blocks/sweep-implausible` endpoint remains for support re-runs. **Still to do (§2):** the sensor-type-aware reset model — see the follow-up item below. Without it we coarse-clamp to 0 (safe, correct for the power-cut case where the true delta is ~0); with it we could re-baseline to the *previous* block's read and recover the true non-zero delta.
...
#### Historical-import standing charges missing / inconsistent  ·  *fix*
*The import doesn't price standing charges reliably.* Reconstructed blocks take the daily standing charge from the Measurements `STANDING_CHARGE_COST` statistic, which is patchy: on the 3.2-test import, **59 days landed at £0.0000** (no standing captured at all), a couple at odd one-off values (0.1287, 0.3780), and the rest at a figure slightly off the settled one (imported **0.5040** vs settled **0.504559**), while live/settled days are correct. Net: the bill's standing-charge total is **understated** across the reconstructed range. Fix: price standing charges from the **tariff standing-charge schedule** (`build_standing_charge_schedule`, already built per agreement) applied **once per day** — exactly as unit rates come from the rate schedule — falling back to it wherever Measurements omits `STANDING_CHARGE_COST`. *Storage note:* both the live and import paths already write the full daily standing charge onto every block in the day and the bill de-dups per-day (MAX), so this isn't over-counting today — but it's fragile; consider normalising to one-block-per-day.

#### ✅ DONE (4.0.x) — Backup **creation** progress feedback  ·  *usability*
*3.5.0 hardened the restore flow with a running/complete banner (and a warn-against-rebuild notice); create-backup got none of it.* When you click **Create backup** there's no signal that anything is happening and no confirmation when it's done — for a multi-MB `blocks.db` (the test backup is ~3 MB zipped, 24 MB raw) that write+zip is not instant, so the button looks dead. Mirror the restore treatment on the create path: an immediate **"Creating backup…"** state, then a **"Backup ready"** completion (with the filename/size and the download), and disable the button while it runs. If creation is moved to a background job like restore, reuse the same job-status polling + banner component rather than a second bespoke one. *Raised 2026-07-22 after producing a manual backup to share.*
> **Resolved:** kept synchronous (a single zip write, no engine pause needed). `/api/backup` now returns the zip **size**; all three backup buttons (Data Management, Delete Blocks, Corrections) show an immediate **"⏳ Backing up…"** state, disable while running, then confirm **"✓ Backup ready: `<filename>` (`<size>`)"** — with the new file appearing in the download list. Added a shared `EMT.fmtBytes` helper (retired the duplicated inline formatter).

#### ✅ DONE (4.0.x) — Delete Blocks background worker + progress banner  ·  *usability*
*Both the preview and the delete run synchronously and block while they work.* On a large reconstructed history the numbers are big — a purge/date-range delete spans **tens of thousands** of blocks (e.g. 33 744 reconstruction blocks below go-live on the 3.2-test DB) plus cascade deletes across `reads` / `generation_mix` / `historical_derivation`, and the **preview** itself does a full `COUNT`/`MIN`/`MAX` scan. Today the button just hangs with no signal, and a long synchronous delete on the engine loop risks the same HA-heartbeat stall → reconnect that the chart-render offload fixed. Mirror the restore/backup-creation treatment: run the preview calc **and** the delete as a **background job** with an immediate **"Calculating… / Deleting…"** banner, live status polling, and a **"Done — N blocks removed"** completion, disabling the controls while it runs. Reuse the same job-status + banner component as restore/backup rather than a third bespoke one, and keep the DB work off the loop thread. *Raised 2026-07-23 while cleaning untagged pre-go-live imports via the date-range delete.*
> **Resolved:** the date-range **delete** and the **purge-all** now run as background workers (own daemon thread, off the engine loop — same store access as before) via a shared `_delete_job` + `/api/blocks/delete/status`, with a persistent banner (step-by-step: deleting → recomputing remainders → rebuilding charts), **poll-on-load so it survives navigation**, a "don't rebuild/restart" warning, disabled controls, a done/error result, and a 409 guard against concurrent ops. *Two follow-ups left (minor):* (1) the **preview** COUNT is still synchronous — quick even on 33k blocks, so not backgrounded; (2) delete uses its own `_delete_job` rather than a single shared job/banner component with restore — consolidation is a refactor, not a fix.

#### Historical carbon for imported blocks — region-correct, opt-in  ·  *feature / correctness*
*Imported history carries no carbon, and the current design deliberately keeps it that way — the question is how to fill it safely.* Evidence from the 3.2-test DB (`20260722T154243`): blocks run back to **2024-07-01** but carbon exists only from **2026-06-04** (2 323 of 36 079 blocks); `carbon_backfill_state` is `{"done": true}` from **2026-07-10** and never re-armed after the **2026-07-22** import. Root cause is intentional: `get_missing_carbon_date_range()` / `get_block_starts_missing_carbon_in_range()` both exclude `source LIKE 'imported%'` ("skip reconstructed"), so the historical Carbon-Intensity backfill won't touch imported blocks. **The address/region risk is real but bounded.** Carbon intensity is **regional** (the Carbon Intensity API keys on postcode → one of ~14 GB DNO regions), while EMT's carbon uses the *manually-configured* postcode (`_get_postcode()`). The import itself records **no** address/region — it filters purely by **MPAN**, and iterates `account.properties` for whichever returns measurements. Crucially an **MPAN is a fixed physical supply point**, so *all* imported data for that MPAN is one location and needs exactly **one** region for the whole span — there's no per-period drift, and it's the same region live carbon already assumes. The only failure mode is the configured carbon postcode not actually matching the MPAN's location. **A single postcode is not enough — we need a region *timeline*.** A house move, or an MPAN/supply-point change, is a **region-change boundary** partway through the imported span, so backfill must resolve *each block's* region from an **effective-dated region history**, not one global postcode. Prerequisite work: a small **region-periods** model (effective-from/to → DNO region, plus `source`: auto-derived vs user-asserted), i.e. a proper mechanism to record **address / meter / region changes over time**. It can reuse the existing effective-dated `config_periods` pattern.

Per source:
- **API import (derivable):** build the timeline automatically from Octopus. `account.properties` already carries **move-in / move-out** dates (`moved_out_at` is read today) and the property **address/postcode** (not yet read) — together these give accurate region(s) and their date boundaries for the imported MPAN(s). Backfill each block against the timeline.
- **CSV import (NOT derivable — the vulnerable case):** a CSV is just kWh/£ with **no MPAN, no address, no region** — it could even span a move or come from a former property. So we **cannot** infer region and must not guess. Either the user **declares the region(s) + date ranges** for the file at import time (feeding the region-periods model), or the CSV blocks are tagged **region-unknown** and **excluded** from carbon backfill (left carbon-NULL).

Plan: **(1)** add the region-periods model + a way to record region/address/meter changes; **(2)** populate it automatically for the API path (property address + tenancy dates), and via an explicit prompt for CSV; **(3)** backfill carbon for imported blocks **only** where the block's region resolves from the timeline — lift the `imported%` exclusion for exactly those; **(4)** any block whose region is unknown or disputed stays **carbon-NULL** with a one-line "confirm your supply region for historical carbon" notice — showing "no data" beats showing wrong data. Opt-in throughout. *May span more than one release — the region-timeline groundwork is the gating piece.*
...
#### #270 — Pick a nearby rate when correcting  ·  *usability*
*Prevents a class of manual-correction error.* The rate-correction form took a free-typed value, so a user could mistype, put the decimal in the wrong place, or round (e.g. `0.05493` → `0.0549`). Fixed by offering the exact rates already in force in blocks **surrounding** the correction window (±3 days), grouped by the stored value and ranked by frequency so the everyday off-peak/peak rates surface first. The user picks one (field fills at full precision) or types their own to override; the list refreshes on date change and pre-fills when a flagged block is loaded from the BL-18 review list. Endpoint: `GET /api/corrections/nearby-rates`. **Built for 3.3.0.**
