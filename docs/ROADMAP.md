...
### 4.2 — Bill-accurate billing (ex-VAT capture + Octopus rounding)

*Turns the 4.1.3 groundwork into the user-facing feature.* Capture the ex-VAT cost at import (**BL-23** — populate the `imp_cost_exc` column, already added and round-tripped by the store, from the `cost_excl` / `value_exc_vat` EMT already fetches, plus a re-import backfill for existing history) and add an opt-in **`bill_rounding: exact (default) | octopus`** totals mode (**BL-24** — route on-read totals through the shipped pure `octopus_bill_total()` ladder), plus an inc/exc **display toggle**. On-read at the totals layer only; default off ⇒ inc-VAT billing stays byte-identical unless a user opts in. Design in `docs/billing_rounding_and_exvat_design.md`.
...
✅ **Shipped 4.1.3 (display-only).** The EV/house split now renders across **Insights → Usage**, **Charts → Usage Stats** (bar + data table + Spiral grouping) and **Charts → Billing** (a bill-breakdown line + a per-day-chart segment) as a pure **display split** — the EV slice is subtracted from the house/Direct segment, so every chart total, the data-table Totals row and the **Total Bill stay byte-identical** — and a **no-op** when a real EV (or any) sub-meter exists. Validated ~99% vs a real CT clamp. The block-store *reversible attribution-source* variant described below was **not** needed to ship the split; it remains the route if the cap (BL-9) later needs to price the EV portion.
...
#### BL-23 — Retain pre-VAT (exc-VAT) figures at import
◑ **Groundwork in 4.1.3.** The additive nullable **`imp_cost_exc`** column is added (idempotent migration) and the store **round-trips** it (write via `_block_rows` + INSERTs, read via `_row_to_block` → channel `cost_exc`); inc-VAT figures are **byte-identical** (verified over a full history — column stays `NULL`, `SUM(imp_cost)` unchanged). **Deferred to 4.2:** populating it at import (threading `cost_excl` / `value_exc_vat` through the engine's pricing branches) + a re-import backfill.

*Low–Medium — plumbing, billing-neutral, no new API calls.* EMT already fetches exc-VAT on every import and discards it: unit rates return `value_exc_vat` (kept: only `value_inc_vat`), and the Measurements query already selects `costExclTax` (`_parse_measurement_node` computes `cost_excl`, but the import reads only `cost_incl`). Add `imp_cost_exc` (+ derived `imp_rate_exc = cost_excl ÷ kWh`) columns, populated at import from data already in hand — additive schema, inc-VAT figures unchanged. Unlocks an ex-VAT display toggle (business/reimbursement) and exact input for BL-24. Existing history is `inc ÷ 1.05` (approx) until a re-import backfills the real exc; live/settled carries it for free once stored. Design in `docs/billing_rounding_and_exvat_design.md`.

#### BL-24 — Opt-in "bill-style rounding" totals mode
◑ **Groundwork in 4.1.3.** The pure **`octopus_bill_total()`** ladder + a Decimal-based **`bankers_round()`** are implemented and unit-tested, but **wired to nothing** (the live billing path is unchanged). **Deferred to 4.2:** the opt-in `bill_rounding: exact | octopus` totals-layer setting + inc/exc display toggle.

*Low — accuracy of *presentation*, opt-in.* A `bill_rounding: exact (default) | octopus` setting that applies Octopus's reverse-engineered rounding ladder — consumption→0.01 kWh (round-half-even), ×exc-rate→0.01p (half-even), sum→whole penny — **on-read at the totals layer only**. Never mutates stored per-slot data; reversible; no reimport; default `exact` so no one's figures move unless they opt in. Core is a pure `octopus_bill_total()` helper (a **Decimal-based** `bankers_round()` — float `round()` mis-handles exact decimal halves like 0.015, which reads as 0.0149… in binary). Runs on the stored exc from **BL-23**; validated to move toward the real bill (26 Jul: raw −106.94p → ladder −108.0p vs calc −108.67p — residual is a consumption-snapshot difference, not rounding). Label it "matches Octopus's *method*", not "penny-perfect". Design in `docs/billing_rounding_and_exvat_design.md`.
