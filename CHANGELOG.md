...
- **Switching the billing chart to Quarter or Year froze for 15+ seconds.** Each period section builds its per-day charts on demand, but it built **every** chart in the section at once — and a quarter holds ~90 days, a year ~365. Selecting those modes rendered hundreds of Plotly charts in one blocking pass, so the whole page locked up and the mode button only showed its pressed state *after* the render finished (making it look like nothing had happened). The daily-charts panel now defaults **collapsed** for Quarter and Year (the bill summary shows immediately), and per-day charts build **lazily only when their panel is expanded** — so switching modes is now instant. Month view is unchanged (current and previous periods still open by default).
