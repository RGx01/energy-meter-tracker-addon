...
- **`started`-dispatch derivation + intelligent-state capture (observe-only, groundwork for [#253](https://github.com/RGx01/energy-meter-tracker-addon/issues/253)).** EMT now fetches the charging device's intelligent control state (`SMART_CONTROL_IN_PROGRESS` / `_CAPABLE` / `_NOT_AVAILABLE`) and derives *started* dispatches the way the reference integration does: the current 30-minute slot is marked `started` only when the state is `SMART_CONTROL_IN_PROGRESS` *and* a planned dispatch is active — accumulated into `dispatch_history`. This is the real smart-vs-bump discriminator (a bump never enters that state, so it never starts), which completed energy alone can't provide. The state fetch is isolated so a schema mismatch can't affect anything else, and nothing here touches billing.
- **Local dispatch accumulation (observe-only, groundwork for [#253](https://github.com/RGx01/energy-meter-tracker-addon/issues/253)).** A new `dispatch_history` table retains every planned and completed dispatch EMT sees, accumulating across polls (first-seen preserved, last-seen refreshed) rather than relying on a single per-poll snapshot. This mirrors how the reference integration keeps dispatch history locally, so transient/tail dispatches aren't lost and a future "this slot never dispatched" judgement becomes trustworthy. Entirely separate from the billing path — nothing reads it to price anything.
