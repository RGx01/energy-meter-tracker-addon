"""C2 — the settled EV/house split comes from the BILL's four buckets, not the dispatch
overlay (design §4). Regression: a completed dispatch that lands AFTER a block is priced
leaves imp_kwh_ev NULL; without the bill split the block settles house-only and orphans the
EV charge. apply_measured_to_block must prefer the cached breakdown's ev_kwh."""
