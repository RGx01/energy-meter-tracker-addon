...
class TestApiImportPlan(unittest.TestCase):
    ...
    def test_empty_store_plans_up_to_now(self):
        # Was `test_no_go_live`: an empty store used to refuse. A fresh API-only
        # install has no blocks to tile up to, so the ceiling is now instead.
        # See tests/test_fresh_install_import.py for the full coverage.
...
def _fresh_empty_store():
    """A real, EMPTY BlockStore — a fresh install before any block exists."""
...
class TestApiImportApply(unittest.TestCase):
    ...
    def test_empty_store_no_longer_refuses(self):
        # Was `test_no_go_live`. A fresh install has no ceiling to tile up to; the
        # import now runs against a ceiling of now rather than refusing outright.
        engine._store = _fresh_empty_store()
        r = asyncio.run(engine.import_api_history(dry_run=True))
